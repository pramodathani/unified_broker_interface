...
Stoxkart's "feed" is a REST poll, not a websocket, because Stoxkart's documented quote websocket could not be reached. The ticks it writes have the same shape as every other broker's, so the normalizer is an ordinary `TickNormalizer`. The measurements behind its attributes are in the module docstring of `stock_brokers/instruments/ticks/stoxkart.py`.

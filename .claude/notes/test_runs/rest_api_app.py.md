...
The Orders tab calls `GET /api/orders/details` and `GET /api/orders/trades`, and the Portfolio tab calls `GET /api/portfolio/funds`, `holdings` and `positions`. All five routes answer from a document that a `bin/unified/` script keeps in Redis, so a call costs one Redis read on the server and never reaches a broker. The page deliberately has no controls for `POST /api/orders/place`, `PUT /api/orders/modify` or `DELETE /api/orders/cancel`, because those reach real trading accounts. `dry_run` would make a write safe, but a page that holds a button one checkbox away from a live order is not worth the convenience.

...
At 23:25 IST the same evening, with the user's permission, the branch's routes placed an after-market MARKET buy of one KWIL share at Zerodha with `market_protection=-1`, in-process against the live Redis with every other broker excluded. Kite accepted it as order `2099920159803219968` in 79 ms, and its order book stored it as a LIMIT order at ₹41.65, about 1% above the last price, with `market_protection` 0: Zerodha applies the protection by converting the order to a limit order at the band's edge. The helper had planned a quantity change that expected `market_protection` in the modify request, and skipped it because the stored order was now LIMIT, which is the route behaving correctly. The order was cancelled in 46 ms and ended `CANCELLED` with nothing filled.

At 23:30 IST, at the user's request, the running API's code placed an after-market SL-M buy of one KWIL share at Zerodha with a trigger of ₹48, in-process in the same way. Kite accepted it as order `2099921390885969920` in 82 ms and stored it as an SL order with a limit price of ₹48.96, 2% beyond the trigger, and `market_protection` 0. A trigger price change to ₹47.50 was sent as an SL modification restating that price, accepted in 49 ms, and seen in Redis within half a second with Zerodha's status `MODIFY AMO REQ RECEIVED` already normalized to `PENDING` by the status fix of that evening. The order was cancelled in 46 ms with nothing filled.

...
|---|---|---|
| <span class="method post">POST</span> | [`/api/orders/flatten`](#flatten) | Cancels every open order, waits, then closes every position |
...
Give the client a generous timeout. The route sends its requests one after another, and it waits up to `UNIFIED_BROKER_INTERFACE_API_ORDER_FLATTEN_WAIT_SECONDS` (5 seconds by default) for the cancels to be confirmed.
...
The "cancel never confirmed" example is shortened: its `closed` entry also carries the position fields shown in the "Flat" example.

### Response attributes

A dry run answers with `would_cancel` and `would_close`. A real run answers with `cancelled`, `still_open_after_waiting`, `closed` and `flat`.
...
| `flat` | boolean | `true` only when every cancel was sent, no order was still open after the wait, and every close was sent and accepted. |
...
| <span class="status s2">207</span> | Some part failed: a cancel could not be sent, an order was still live after the wait, or a close was not sent or not accepted. `flat` is `false`. Read every entry. |
...
??? note "Under the hood"
    ...
    - **Timing:** the wait length is `UNIFIED_BROKER_INTERFACE_API_ORDER_FLATTEN_WAIT_SECONDS` (default `5`), re-reading every 0.25 seconds. A Redis error during a re-read is skipped and the wait goes on.

...
def _application_key(broker_name, application):
    """
    Redis key holding a shared session for one of a broker's secondary applications.

    - `broker_name` is the name of the broker.
    - `application` names the application within that broker, such as `marketdata`.
    """
    return f"ubi:session:{broker_name}:{application}"

def shared_application_session(broker_name, application, log_in, logger=None, refresh=False,
                              timeout_seconds=LOCK_TIMEOUT_SECONDS):
    """
    A session for a broker application that issues one session at a time, shared between processes.

    Some brokers split a login into more than one application, and issue exactly one session per
    application: Wisdom Capital runs Symphony's XTS, whose market data credentials are separate
    from its trading ones, and logging in a second time invalidates the first token rather than
    handing out a second. Two processes that each log in therefore take turns invalidating each
    other's session.

    This is `ensure_session` applied one level down: whoever finds no stored session takes a Redis
    lock, logs in once, and publishes the result for everyone else to read.

    Returns whatever `log_in` returned, either freshly or from the cache.

    - `broker_name` is the name of the broker.
    - `application` names the application within it, and is part of the Redis key.
    - `log_in` is called to perform the login, and must return a JSON serialisable object.
    - `logger` is where progress is reported. A named logger is made when not supplied.
    - `refresh` logs in even when a session is stored, for a session that has stopped working.
    - `timeout_seconds` is how long the stored session is kept before it is minted again.
    """
    import json

    logger = logger or get_logger(f"{broker_name}.session")
    cache = get_cache()
    key = _application_key(broker_name, application)

    if not refresh:
        stored = cache.get(key)
        if stored:
            logger.info(f"Using the shared {broker_name} {application} session.")
            return json.loads(stored)

    lock = f"{key}:lock"
    deadline = time.time() + WAIT_FOR_OTHER_SECONDS
    while not cache.set(lock, str(os.getpid()), nx=True, ex=LOCK_TIMEOUT_SECONDS):
        # Someone else is logging in. Wait for what they publish rather than logging in too,
        # because a second login would invalidate the token they are about to store.
        time.sleep(2)
        stored = cache.get(key)
        if stored:
            return json.loads(stored)
        if time.time() > deadline:
            raise SessionException(
                f"Timed out waiting for another process to establish the {broker_name} "
                f"{application} session.")

    try:
        logger.info(f"Establishing the {broker_name} {application} session.")
        session = log_in()
        # The expiry is what makes a token that the broker has quietly retired eventually get
        # minted again, rather than being served from Redis until someone notices.
        cache.set(key, json.dumps(session), ex=timeout_seconds)
        return session
    finally:
        cache.delete(lock)

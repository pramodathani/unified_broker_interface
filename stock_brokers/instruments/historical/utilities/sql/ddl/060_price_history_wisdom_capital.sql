-- Historical candles for wisdom_capital, exactly as the broker serves them.
--
...
--
-- wisdom_capital serves history from around 2020.

CREATE TABLE IF NOT EXISTS wisdom_capital.price_history (
...
ALTER TABLE wisdom_capital.price_history SET (
...
-- upsert.
CREATE TABLE IF NOT EXISTS wisdom_capital.price_history_progress (
    instrument_token      TEXT      NOT NULL,
    "interval"            TEXT      NOT NULL,
...
CREATE INDEX IF NOT EXISTS price_history_progress_claim_wisdom_capital
    ON wisdom_capital.price_history_progress (priority, next_attempt_after NULLS FIRST)
    WHERE reached_broker_limit = FALSE;

CREATE INDEX IF NOT EXISTS price_history_progress_outcome_wisdom_capital
    ON wisdom_capital.price_history_progress (last_outcome, last_attempt_at DESC);

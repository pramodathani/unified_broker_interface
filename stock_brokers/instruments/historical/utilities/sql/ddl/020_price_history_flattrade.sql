-- Historical candles for flattrade, exactly as the broker serves them.
--
...
--
-- flattrade serves history from around 2012.

CREATE TABLE IF NOT EXISTS flattrade.price_history (
...
ALTER TABLE flattrade.price_history SET (
...
-- upsert.
CREATE TABLE IF NOT EXISTS flattrade.price_history_progress (
    instrument_token      TEXT      NOT NULL,
    "interval"            TEXT      NOT NULL,
...
CREATE INDEX IF NOT EXISTS price_history_progress_claim_flattrade
    ON flattrade.price_history_progress (priority, next_attempt_after NULLS FIRST)
    WHERE reached_broker_limit = FALSE;

CREATE INDEX IF NOT EXISTS price_history_progress_outcome_flattrade
    ON flattrade.price_history_progress (last_outcome, last_attempt_at DESC);

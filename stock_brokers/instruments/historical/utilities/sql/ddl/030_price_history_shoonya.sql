-- Historical candles for shoonya, exactly as the broker serves them.
--
...
--
-- shoonya serves history from around 2010.

CREATE TABLE IF NOT EXISTS shoonya.price_history (
...
ALTER TABLE shoonya.price_history SET (
...
-- upsert.
CREATE TABLE IF NOT EXISTS shoonya.price_history_progress (
    instrument_token      TEXT      NOT NULL,
    "interval"            TEXT      NOT NULL,
...
CREATE INDEX IF NOT EXISTS price_history_progress_claim_shoonya
    ON shoonya.price_history_progress (priority, next_attempt_after NULLS FIRST)
    WHERE reached_broker_limit = FALSE;

CREATE INDEX IF NOT EXISTS price_history_progress_outcome_shoonya
    ON shoonya.price_history_progress (last_outcome, last_attempt_at DESC);

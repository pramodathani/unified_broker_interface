-- Historical candles for indmoney, exactly as the broker serves them.
--
...
--
-- indmoney serves history from around 2010.

CREATE TABLE IF NOT EXISTS indmoney.price_history (
...
ALTER TABLE indmoney.price_history SET (
...
-- upsert.
CREATE TABLE IF NOT EXISTS indmoney.price_history_progress (
    instrument_token      TEXT      NOT NULL,
    "interval"            TEXT      NOT NULL,
...
CREATE INDEX IF NOT EXISTS price_history_progress_claim_indmoney
    ON indmoney.price_history_progress (priority, next_attempt_after NULLS FIRST)
    WHERE reached_broker_limit = FALSE;

CREATE INDEX IF NOT EXISTS price_history_progress_outcome_indmoney
    ON indmoney.price_history_progress (last_outcome, last_attempt_at DESC);

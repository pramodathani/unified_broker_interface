-- Historical candles for dhan, exactly as the broker serves them.
--
...
CREATE TABLE IF NOT EXISTS dhan.price_history (
...
ALTER TABLE dhan.price_history SET (
...
-- upsert.
CREATE TABLE IF NOT EXISTS dhan.price_history_progress (
    instrument_token      TEXT      NOT NULL,
    "interval"            TEXT      NOT NULL,
...
CREATE INDEX IF NOT EXISTS price_history_progress_claim_dhan
    ON dhan.price_history_progress (priority, next_attempt_after NULLS FIRST)
    WHERE reached_broker_limit = FALSE;

CREATE INDEX IF NOT EXISTS price_history_progress_outcome_dhan
    ON dhan.price_history_progress (last_outcome, last_attempt_at DESC);

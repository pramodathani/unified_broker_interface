-- Historical candles for zerodha, exactly as the broker serves them.
--
...
--
-- zerodha serves history from around 2005.

CREATE TABLE IF NOT EXISTS zerodha.price_history (
...
ALTER TABLE zerodha.price_history SET (
...
-- upsert.
CREATE TABLE IF NOT EXISTS zerodha.price_history_progress (
    instrument_token      TEXT      NOT NULL,
    "interval"            TEXT      NOT NULL,
...
CREATE INDEX IF NOT EXISTS price_history_progress_claim_zerodha
    ON zerodha.price_history_progress (priority, next_attempt_after NULLS FIRST)
    WHERE reached_broker_limit = FALSE;

CREATE INDEX IF NOT EXISTS price_history_progress_outcome_zerodha
    ON zerodha.price_history_progress (last_outcome, last_attempt_at DESC);

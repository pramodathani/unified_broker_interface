-- The live stream tables of wisdom_capital: wisdom_capital.ticks, wisdom_capital.order_updates and wisdom_capital.positions.
--
-- Written by the persisters in bin/wisdom_capital/ - persist_ticks from wisdom_capital:quotes:stream, persist_orders from
-- wisdom_capital:order-updates:stream, persist_positions from wisdom_capital:positions_updates:stream -
...
CREATE TABLE IF NOT EXISTS wisdom_capital.ticks (
...
CREATE INDEX IF NOT EXISTS ticks_id_time_idx
    ON wisdom_capital.ticks (id, "time" DESC);

-- Compressed by id, so a compressed chunk still serves reads of one id.
ALTER TABLE wisdom_capital.ticks SET (
...
CREATE TABLE IF NOT EXISTS wisdom_capital.order_updates (
    time                timestamptz   not null,
...
-- One row per position snapshot the position stream sent.
CREATE TABLE IF NOT EXISTS wisdom_capital.positions (
    time                timestamptz   not null,
    id                  text          not null,
    instrument_token    text,
    tradingsymbol       text,
...
-- A trading day in one chunk, the unit most queries scan and the unit compression acts on.
SELECT create_hypertable(
    'wisdom_capital.positions',
    by_range('time', INTERVAL '1 day'),
    if_not_exists => TRUE
);

-- One instrument's positions over a period, most recent first.
CREATE INDEX IF NOT EXISTS positions_id_time_idx
    ON wisdom_capital.positions (id, "time" DESC);

-- Compressed by id, so a compressed chunk still serves reads of one id.
ALTER TABLE wisdom_capital.positions SET (
    timescaledb.enable_columnstore = true,
    timescaledb.segmentby = 'id',
    timescaledb.orderby = '"time" DESC'
);

-- The live stream tables of fyers: fyers.ticks, fyers.order_updates and fyers.positions.
--
-- Written by the persisters in bin/fyers/ - persist_ticks from fyers:quotes:stream, persist_orders from
-- fyers:order-updates:stream, persist_positions from fyers:positions_updates:stream -
...
CREATE TABLE IF NOT EXISTS fyers.ticks (
...
CREATE TABLE IF NOT EXISTS fyers.order_updates (
    time                timestamptz   not null,
...
CREATE TABLE IF NOT EXISTS fyers.positions (
    time                timestamptz   not null,
    id                  text          not null,
    instrument_token    text,
    tradingsymbol       text,
...
CREATE INDEX IF NOT EXISTS positions_id_time_idx
    ON fyers.positions (id, "time" DESC);

-- Compressed by id, so a compressed chunk still serves reads of one id.
ALTER TABLE fyers.positions SET (
    timescaledb.enable_columnstore = true,
    timescaledb.segmentby = 'id',
    timescaledb.orderby = '"time" DESC'
);

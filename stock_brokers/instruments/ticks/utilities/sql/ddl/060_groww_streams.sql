-- The live stream tables of groww: groww.ticks, groww.order_updates and groww.positions.
--
-- Written by the persisters in bin/groww/ - persist_ticks from groww:quotes:stream, persist_orders from
-- groww:order-updates:stream, persist_positions from groww:positions_updates:stream -
...
CREATE TABLE IF NOT EXISTS groww.ticks (
...
CREATE TABLE IF NOT EXISTS groww.order_updates (
    time                timestamptz   not null,
...
CREATE TABLE IF NOT EXISTS groww.positions (
    time                timestamptz   not null,
    id                  text          not null,
    instrument_token    text,
    tradingsymbol       text,
...
CREATE INDEX IF NOT EXISTS positions_id_time_idx
    ON groww.positions (id, "time" DESC);

-- Compressed by id, so a compressed chunk still serves reads of one id.
ALTER TABLE groww.positions SET (
    timescaledb.enable_columnstore = true,
    timescaledb.segmentby = 'id',
    timescaledb.orderby = '"time" DESC'
);
